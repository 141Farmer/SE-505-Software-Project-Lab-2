...
# @payment_router.post("/")
# def redirect_to_gateway(request: Request, current_user = Depends(AuthHandler.get_current_user)):#paymentAmount: float, current_user = Depends(AuthHandler.get_current_user)):

    
#     # url = str(request.url)
    
#     # if not url.startswith("http"):
#     #     corrected_url = f"http://{request.url.hostname}{request.url.path}"
#     #     return RedirectResponse(corrected_url, status_code=308)  # Permanent Redirect

#     tran_id = uuid4()
#     data = {
#         'total_amount': f"{100000}",
#         'currency': "BDT",
#         'tran_id': f"{tran_id}",
